...
def init_rasterfiles(datapath, preloadtypes):
    ...
    try:
        ...
        message = 'Could not initialize raster files. Make sure they exist in the following '\
            'directory: {} (Exception: {})'.format(datapath, e)
        log.error(message)
...
class BTTile(Tile):
    ...
    def getVal(self, x, y):
        ...
        if not hasattr(self, 'cols'):
            file.seek(10)
            (self.cols, self.rows, self.dataSize, self.floatingPoint) = unpack(
                '<LLhh', file.read(12))
