...
def loadShapefile(file_name):
    ...
    db = list(dbfutils.dbfreader(dbf))  # noqa
    dbf.close()
    fp = open(file_name, 'rb')

    # get basic shapefile configuration
    fp.seek(32)
    shp_type = readAndUnpack('i', fp.read(4))  # noqa
    shp_bounding_box = readBoundingBox(fp)  # noqa
...
def createRecord(fp):
    ...
    if record_number == b'':
        return False
    content_length = readAndUnpack('>L', fp.read(4))  # noqa
...
def readRecordPolyLine(fp):
    ...
    for part_index in range(0, data['numparts']):
        point_index = data['parts'][part_index]  # noqa
        ...
        # while( ! in_array( points_read, data['parts']) and points_read <
        # data['numpoints'] and !feof(fp)):
        checkPoint = []
    ...
    for i in range(len(records)):
        ...
        for j in range(i + 1, len(records)):
            ...
            if overlap(records[i], records[j]):
                # if so, check every single point in this feature to see if it matches a
                # point in the other feature
