...
class TestFileView(TestsBase):
    ...
    def test_options(self):
        resp = self.testapp.options('/geoportalxyz/files', status=200)
        ...
        resp = self.testapp.options('/geoportalxyz/files/toto', status=200)
        self.assertEqual(resp.headers['Access-Control-Allow-Methods'], 'POST,GET,DELETE,OPTIONS')
        self.assertEqual(resp.headers['Access-Control-Allow-Credentials'], 'true')
    ...
    def test_create_get_kml(self):
        ...
        resp = self.testapp.get('/geoportalxyz/files/{}'.format(admin_id), status=200)
        self.assertIn('fileId', resp.json)

        # Get with file id
        file_id = resp.json['fileId']
        resp = self.testapp.get('/geoportalxyz/files/{}'.format(file_id), status=200)
    ...
    def test_inexistant_id(self):
        self.testapp.get('/geoportalxyz/files/toto', status=404)
        self.testapp.delete('/geoportalxyz/files/toto', status=404)

    def test_file_invalid_content_type(self):
        headers = {'Content-Type': WRONG_CONTENT_TYPE}
        self.testapp.post('/geoportalxyz/files', VALID_KML, headers=headers, status=415)
    ...
    def test_update_kml(self):
        ...
        url = '/geoportalxyz/files/%s' % admin_id
    ...
    def test_forked_kml(self):
        ...
        url = '/geoportalxyz/files/%s' % file_id
        resp = self.testapp.post(url, VALID_KML, headers=self.headers, status=200)
    ...
    def test_delete_kml(self):
        ...
        # delete with file_id
        resp = self.testapp.delete(
            '/geoportalxyz/files/%s' % file_id,
            headers=self.headers,
            status=401)

        # Delete with admin_id
        resp = self.testapp.delete(
            '/geoportalxyz/files/%s' % admin_id,
            headers=self.headers,
            status=200)
        self.assertTrue(resp.json['success'])

        # Delete non existent file (this is not an admin_id os this is not authorized)
        url = '/geoportalxyz/files/%s' % 'this-file-is-nothing'
    ...
    def test_file_too_big_kml(self):
        ...
        with open(os.path.join(current_dir, 'big.kml')) as f:
            data = f.read()
        resp = self.testapp.post('/geoportalxyz/files', data, headers=self.headers, status=413)
    ...
    def test_file_ie9_fix(self):
        ...
        self.testapp.post('/geoportalxyz/files', URLENCODED_KML, headers={}, status=415)
